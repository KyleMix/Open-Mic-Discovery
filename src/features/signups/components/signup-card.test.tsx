...
// The window states depend on the wall clock; pinned before the earliest
// opening in the fixtures so "not yet open" stays true no matter when the
// suite runs.
beforeAll(() => {
  jest.useFakeTimers({ now: new Date('2026-08-01T19:00:00Z'), doNotFake: ['queueMicrotask'] });
});
afterAll(() => {
  jest.useRealTimers();
});
...
describe('when signups have not opened yet', () => {
  ...
  it('keeps the date when signups open on an earlier day', async () => {
    ...
    expect([text.props.children].flat(Infinity).join('')).toMatch(/Monday, Aug 3 at 7:00\s?PM/i);
  });
});
