-- Posters in discovery.
--
-- Producers can already upload a poster (shown on the mic page). Surfacing
-- it on the Discover list and Favorites gives each mic a face, so mics_near
-- gains poster_url. The return type changes, so the old signature drops
-- first. Body is otherwise identical to 000700 (SECURITY INVOKER, RLS
-- applies).
