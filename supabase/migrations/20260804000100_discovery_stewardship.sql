-- PROPOSAL (DECISIONS_NEEDED item 12, approved): expose owner_id from the
-- two discovery RPCs so cards can show host-managed vs community-listed.
-- The only change to each function is the appended owner_id column; every
-- other column, filter, and ordering clause is copied verbatim from
-- 20260803000600 (mics_near) and 20260728000700 (search_mics). Adding a
-- column to "returns table" changes the return type, which CREATE OR
-- REPLACE refuses, so both functions are dropped and recreated inside one
-- transaction, then re-granted.
--
-- NOT YET APPLIED. The client reads owner_id optionally and renders no
-- badge when the column is absent, so the app behaves identically before
-- and after this lands. After applying, regenerate the Supabase types.
