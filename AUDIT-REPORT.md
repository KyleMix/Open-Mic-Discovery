...
- [x] (Done 2026-08-11) `docs/privacy/SDK_MANIFEST_AUDIT.md` re-audited from the installed tree: the `react-native-purchases` row is gone (with an explicit do-not-declare note), versions match package-lock (RN 0.86.2, reanimated 4.5.1, worklets 0.10.1), and the added native-bearing deps are covered (`expo-media-library`, `react-native-view-shot`, `expo-clipboard`, `expo-haptics`, `expo-network`, `expo-sharing`, `expo-updates`), every bundled manifest re-read rather than carried forward.
- [x] (Done 2026-08-11) `docs/store/SUBMISSION_CHECKLIST.md` stamped superseded by `LAUNCH-CHECKLIST.md` at the top, and the values that could be copied into a form were corrected in place (Explorer name, `com.openmicexplorer.app`, `openmicexplorer://` scheme, current test counts, RevenueCat secrets removed, leftover IAP fragments deleted). `REVIEW_NOTES.md:7` now points at `docs/LAUNCH-CHECKLIST.md`.
- [x] (Done 2026-08-11) `docs/store/ACCOUNT_DELETION_PAGE.md` rewritten as a pointer to the shipped page (`web/delete-account/index.html`, self-service, immediate); the 7-day email flow and subscription-history text are gone.
- [x] (Done 2026-08-11) `docs/APP_BREAKDOWN.md` carries a dated-snapshot banner and its name, pin, and monetization claims are corrected; `docs/pitch/INVESTOR_PITCH.md` now states that the Producer Pro stack was built and then removed from the launch build (owner decision 2026-07-30), with the paywall screenshot and "monetization live in code" claims removed.
