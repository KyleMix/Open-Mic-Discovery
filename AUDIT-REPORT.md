...
### 3a. Missing indexes (SQL: `docs/audit/proposed/20260811000100_search_and_queue_indexes.sql`)
Additive `CREATE INDEX` only; cannot change results, only cost. Validated to create cleanly against the migrated schema. Do not drop into `supabase/migrations/` until reviewed; prefer `CREATE INDEX CONCURRENTLY` on the hosted project.
...
- Real, low severity: `profiles owner update` lets a user self-set `deleted_at` (verified by probe: the write succeeds), which then locks their own profile out of every later update, a state `delete_account()` never produces; the same policy lets a user self-set `is_producer`/`is_performer`, bypassing the `review_claim` grant path. Fix: pin `deleted_at` and the role columns in the existing profile guard trigger for non-admin writes.
- Real, medium: banned and unapproved host stage names are readable by anon through `series_search`. `build_series_search` filters the host join only on `deleted_at is null`, and `profiles_search_sync` never fires on `moderation_status`, so a banned host's stage name (the one identifier `public_profiles` suppresses) is returned by `select fuzzy from series_search`. Fix: gate the host contribution on approved-and-not-banned and resync on `moderation_status`.
- Defense in depth: `revoke execute` on `private.delete_account_for(uuid)` from `anon, authenticated` (safe today only because `private` is not PostgREST-exposed), and revoke the blanket default write grants on `connections`, `share_events`, `series_search`, `mic_credits`, `attendance_plans`, matching what `report_triage` and `user_sanctions` already do.
