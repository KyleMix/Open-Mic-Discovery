...
| Test kit | Complete                | 2026-08-01 | Owner setup and testing tools. Owner bootstrap: signing in with kylewmixon@gmail.com on any environment (local, preview, hosted) now lands a performer + producer + admin account, already approved, with both child profile rows, via a database trigger plus backfill. Test kit: admin-only RPCs that build a whole situation in one call (mic of yours starting in 90 minutes with a roster, lottery waiting on the draw, a week of listings across every discipline and price, unclaimed listing plus pending claim, three freshness states, full moderation queue, your own account on three lists), a time machine that moves any night to minutes from now (DST-correct, local date recomputed in the series timezone), a roster filler that works on real listings too, role switches, a device-only Producer Pro override, and a reset that removes exactly what the kit created (registry-tracked, pgTAP-verified against before/after table counts). Kill switch for store submission. Migration 20260801000100; 156 pgTAP, 90 Jest. |
