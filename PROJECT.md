...
| Testing | Complete, awaiting review | 2026-08-01 | CI added (.github/workflows/ci.yml): typecheck, lint, unit tests, the unit suite again under a second timezone, and Supabase up plus `supabase test db`. Nothing ran on push before this. The pgTAP suite's pass state had never been confirmed; it passes. TESTING_STRATEGY.md records the per-layer argument, the fake, build order, and what is deliberately uncovered. Authorized source change: all eight update sites now `.select('id')` and throw when zero rows return, because RLS filters denied rows out of an UPDATE and returned `{error: null}`, identical to success. New coverage: eight write sites, the roster Realtime subscription, the favorites screen's five states, plus pgTAP for the four pg_cron jobs (whose scheduling failure is swallowed by an exception handler) and for RLS being the only guard rail under a blanket grant. Two verified defects recorded as `test.failing` because their fixes were out of scope: `sortSoonestNearest` buckets by host timezone rather than the mic's, and the discovery query key captures `view` and `disciplinesSeeded` so toggling map/list refetches and forks the persisted cache. Timezone is pinned in globalSetup, not setupFiles, because Jest sandboxes process.env. 145 pgTAP, 119 Jest. Not covered: Maestro e2e (wired to no runner), AuthGate, the native map, and push-sender (excluded from typecheck and lint, no Deno toolchain). |
