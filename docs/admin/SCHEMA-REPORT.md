...
> **Both admin policies dropped, 2026-08-07** (`20260807001600`). Admins read
> other accounts through `admin_profile_review`, which omits `display_name`,
> `birth_year`, `home_city` and `home_location`. The update policy went with the
> select policy for a reason worth knowing: an UPDATE whose `WHERE` clause reads
> the table needs the SELECT policies satisfied too, so removing the select made
> the update unreachable for anybody else's row (measured: zero rows affected).
> Rather than leave a policy that cannot fire, it is gone. Profile moderation
> goes through `moderate_content`, which is `SECURITY DEFINER` and never
> consulted these policies, so there is now no raw admin write path to `profiles`
...
Reading: owner does everything; admin reads, and cannot write. So an admin could
see a producer's phone number and email with no reason string and no audit row.
That was the single largest S12 gap in the existing schema.

> **Closed, 2026-08-07** (`20260807001600`). `producer profile admin select` is
> dropped. Admins read `admin_producer_review`, where `contact_email` and
> `contact_phone` are masked to a shape (`p***t@domain`, `**** 0142`) and
> `payout_ref` is absent entirely. Unmasking goes through `admin_reveal`, which
> requires owner or moderator, refuses a blank reason, is rate limited to thirty
> an hour per admin, and writes an audit row naming the field and never the
> value, marked not reversible.
...
Admin promotion path as inventoried: the hard-coded array in
`private.owner_emails()`, which returns `array['kylewmixon@gmail.com']`, plus
`email_confirmed_at is not null`, plus F-B (any admin can promote anyone by
REST). There was no allowlist table, no invite, no deactivation, and no record of
who promoted whom.

> **Changed, 2026-08-07.** `admin.admin_users` is now the source of truth, with
> `owner`, `moderator`, and `read_only` on `admin.admin_role`. Promotion is
> invite only through `admin_invite` plus `admin_invite_accept`, both owner
> gated (acceptance is gated by the token and the session's own address), and
> every change appends an audit row. `profiles.is_admin` became a cache with one
> definition, `private.admin_bit_for()`, which is "an active allowlist row whose
> role is owner or moderator", applied by the profiles guard and by a trigger on
> the allowlist so the two cannot drift.
>
> One correction carried from `20260807001300`: its header says `read_only` is a
> valid audit-log actor because S12 wants a row when one of them reveals an
> email. `20260807001600` decided otherwise. Oversight does not need somebody's
> phone number, so revealing requires owner or moderator. `read_only` remains a
> valid actor in the log, because nothing depends on it being otherwise.
>
> The consequence to know: a `read_only` admin gets `is_admin = false`, so the
...
> policies; the two sets turned out to be entirely disjoint.
>
> **The reader split is done, 2026-08-07**, as
> `20260807001500_read_only_admins_can_read.sql`: `private.is_admin_reader()`
> (any active allowlist row, `read_only` included) on twelve SELECT policies,
> while `private.is_admin()` keeps its meaning and stays on every write policy
> and in every privileged RPC. So a `read_only` admin now reads the queue, held
> content, rosters, flags, claims and sanctions, and writes nothing anywhere.
> Four SELECT policies deliberately keep the write predicate: `test_kit_settings`
> and `test_kit_objects` (a facility that mints sign-ins, not the queue),
> `banned_terms` (a setting), and `producer_profiles` (the largest PII surface in
> the schema, which S12 wants masked before it is widened by a role).
...
| S2  | No self signup, invite only from an allowlist                                                                                       | **Not met, and cannot be met by Auth config.** `enable_signup = true` and `auth.email.enable_signup = true` are required by the mobile app. The only "allowlist" is a hard-coded email array in `private.owner_emails()`                                                                                                                                                                                                                                                                                                                                            | **Built 2026-08-07** (`20260807001300_admin_users_and_invites.sql`): `admin.admin_users` and `admin.admin_invites`, invite only, owner managed, every mutation audited. Still needed: The console's own gate: a Supabase user with no active `admin_users` row must get a generic 404, not a 403, so the console's existence is not confirmable                                                                                                                                   |
| S3  | Mandatory TOTP MFA, AAL2 in middleware and in policy                                                                                | **Database half built 2026-08-07** (`20260807001700`), and switched off pending enrolment. The project is on Pro, so TOTP is available; it is enabled for the local stack in `config.toml` and still needs enabling in the dashboard for the hosted one. AAL2 is folded into `private.is_admin()` and `private.is_admin_reader()`, so all 29 admin policies inherit it. Enforcement is gated on `admin.security_settings.require_aal2`, which ships false because requiring AAL2 before anybody has enrolled locks out every admin. Middleware half is console work |
...
| S8  | Step-up reauth within five minutes for destructive actions                                                                          | **Built 2026-08-07** (`20260807001700`), behind the same flag as S3. `admin.require_step_up()` requires a TOTP challenge inside `admin.step_up_window()` (five minutes, hard-coded rather than configurable, because a security window only ever gets turned one way) and is called by `admin.require_owner()` and by `admin_sanction_apply` for a suspension or a ban. A warning is exempt on purpose. One assumption is untested from here: the exact `amr` method string, held in `admin.mfa_methods()`                                                          |
...
| S12 | PII minimization, masking, reason-gated reveal, no SQL runner, no export                                                            | **Met 2026-08-07** (`20260807001600`). Both blanket admin select policies dropped; admins read `admin_profile_review` and `admin_producer_review`, where contact details are masked and `payout_ref` and `home_location` are absent. `admin_reveal` returns one field for one person against a required reason, rate limited, and writes an audit row naming the field and never the value. read_only cannot reveal. No query box and no export are constraints on the console rather than migrations, so those remain to be honoured when it is built              |
...
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `admin_users` (user id, role, active, created by, created at)  | **Done 2026-08-07** as `admin.admin_users`, with the three roles on `admin.admin_role`. It does not replace `private.owner_emails()`: that function is now the bootstrap seed for the allowlist on a fresh database, and the owner's first sign-in inserts their own `owner` row. `profiles.is_admin` became a cache of this table, defined once by `private.admin_bit_for()`                                                                                                                                                                                                                                                                                                              |
| `admin_invites` (email, role, token hash, expiry, consumed at) | **Done 2026-08-07** as `admin.admin_invites`. Only the SHA-256 hash of the token is stored, and `admin_invite()` returns the raw token once. Redeeming needs both the token and a session on the invited address, so a leaked token is not enough. One live invitation per address, enforced by a partial unique index                                                                                                                                                                                                                                                                                                                                                                     |
...
Stated here rather than left to be discovered. **Nothing writes to it.**
`moderate_content`, `resolve_flag`, `review_claim`, and the direct `reports`
update all predate the reason string S6 requires on every row, and adding a
reason parameter changes signatures the mobile app calls. Logging those actions
with a null reason would put a hole in the one guarantee the table exists to
make, so the console's own RPCs will be the first writers. Two consequences:

- Until the console ships, moderation performed from the in-app admin screen
  (`src/app/admin.tsx`) is still unaudited. That is not a regression, it is the
  status quo, and it is another reason open question 5 (remove the app screen or
  keep it read only) wants an answer.
- The table's contents are not in `src/types/database.types.ts`, because
  `scripts/dev/gen-types.mjs` generates for `public` only. That is correct for
  the app, which must never touch this schema. The console generates its own
  types and will need `admin` in its schema list.
...
2. **App repo**: `admin_users`, `admin_invites`, `audit_log` (outside `public`),
   ...
   rule. Two thirds done as of 2026-08-07:
   - `reports` extension: **done**, `20260807001100_report_triage.sql`.
   - `admin.audit_log`: **done**, `20260807001200_audit_log.sql`. Nothing writes
     to it yet, and the reason is in section 9.
   - `admin_users` and `admin_invites`: **done**,
     `20260807001300_admin_users_and_invites.sql`, with the five owner-only
     RPCs and `admin.actor_role()` switched over to read the allowlist.
   - Still open: the custom access token hook, and with it the AAL2 predicates
...
3. ~~**App repo**: `user_sanctions` and `private.can_act()`, threaded through the
   ...
   The `read_only` reader predicate on the SELECT policies is **also done**,
   `20260807001500_read_only_admins_can_read.sql`. Still open: the app-side work
   below.
...
2. ~~**Audit log**: one `audit_log` with a reversal self-reference, or the brief's
   two tables (`audit_log` plus `moderation_actions`)?~~ **Answered: one table
   with the reversal self-reference.** Done, see section 9.
