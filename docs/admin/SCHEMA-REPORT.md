...
2. **App repo**: `admin_users`, `admin_invites`, `audit_log` (outside `public`),
   ...
   - Still open: `admin_users`, `admin_invites`, the custom access token hook.
     `admin.admin_role` already exists and `admin_users` should use it.
     `admin.actor_role()` is the one place that derives a role today, from the
     owner email and `is_admin`, and it is the function that changes when the
     allowlist lands.
