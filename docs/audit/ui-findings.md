...
Status after the fix and polish passes: every finding is resolved or has a recorded decision. Scored items: all fixed; the 6-destination tab bar was resolved by moving Network under the Profile tab (5 destinations remain; the request badge moved to the Profile tab icon). Polish pass: M-2 (reset-password now uses FormScreen), M-3 (every sheet dismisses on backdrop tap and cross-fades under Reduce Motion), M-4 (all chips share radius.pill), M-5 (body and caption text now carry fonts.regular app-wide), M-6 (screens and lists share one 24pt horizontal padding), M-9 (running text and legal text scale to maxReadingFontScale 2.5x while chrome keeps the 1.6x cap; a full-range 3.1x pass still wants a device check), M-10 (stack transitions and the lottery shuffle respect Reduce Motion), M-17 (interactive boundaries use palette.borderInput #6A6A76, 3.37:1 on bgElevated; decorative card borders stay quiet by design), P-1 (radius.sheet token), P-2 (shared SheetGrabber), P-6 (credits page pads 24), P-7 (toast dismiss uses the Ionicons close). Deliberately kept: P-4 (skeleton artwork literals) and P-5 (three icon systems), both documented as intentional. Line numbers below refer to commit `0739d0d`, before the fixes.
