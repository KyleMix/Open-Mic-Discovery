...
| 6 | Onboarding is a trap; partial failure gives unwinnable handle error | src/app/(auth)/onboarding.tsx, src/features/auth/api.ts | Sign-out escape button; EULA redirect when the in-memory acceptance is gone; profile pre-check + upsert-on-conflict make resubmits safe | in progress |
