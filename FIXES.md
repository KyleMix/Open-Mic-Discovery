...
| 47 | free-token strips names; multi-day tokens drop; guest area note suppressed | Word tokens bound on whitespace (Free-For-All survives); day tokens accumulate and beat the when conflict; guests see the default-area note | done |
| 48 | Missing retries; producer not-found vs error; analytics zero totals; null-on-error subcomponents | Retries on profile nights, blocked list, upcoming nights, share sheet; Go live and Who is coming own their errors; analytics totals only with nights; manage not-found is an escape, not a retry loop | done |
| 49 | Return-to family (dup stack, dropped params, inert guard, unguarded back, push verb, swallowed signOut, bare /open-mics) | dismissTo collapses the funnel; credits records its occurrence param; guard matches real pathnames (tested); canGoBack fallback; navigate not push for tab switches; EULA sign-out failure surfaces; bare /open-mics handled | done |
| 50 | Copy: slot/spot, listing/mic, event history, ellipses, My Mics case, screen self-naming, method label maps scattered | spot/slot, mic/listing, signup history, ellipses, My mics case, screen self-names unified; all three method copy maps live in one file | done |
