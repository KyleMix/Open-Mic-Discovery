...
- **Signup lifecycle.** First-come and reserved-slot mics confirm instantly in arrival order until capacity, then waitlist. Lottery mics hold entries until the host draws; the draw randomizes server side with a visible shuffle, fills capacity slots, and waitlists the rest. The draw is a one-time act: once anyone has been marked performed or no-show the night is underway and the draw closes, because renumbering the people still waiting would collide with the positions the ones who already went up are holding. Reordering and waitlist promotion stay available and keep everyone else in place. Status changes push notify the performer (subject to their preferences).
...
- **Lineup (host and featured artist).** Manage screen, Lineup: set who normally hosts and who is featured. Either can be a linked Open Mic Explorer account (searched by name or handle) or typed in by hand with their own socials. A linked credit takes its name and links from that profile, so they stay current when the person edits it; a typed one carries its own. Upcoming nights each have a Lineup button that overrides just that night, leaving every other night on the series default, and the listing labels the difference ("Featured tonight"). Names go through the same banned-term filter as titles. Socials render as the platform logos, now including Spotify and Apple Music, which are also on every profile.
...
- **Running the night.** Live keeps three numbers on screen the whole time: spots still open against the cap, how many of the list have gone up, and how many are on the waitlist. They stay after the last name is done, because "can I put one more person on" is exactly the question at that point. The set timer counts the mic's own allotted set length down and then counts the overrun up, silently (colour plus one haptic tap). Waitlisted performers can be added to the list from the Live screen itself rather than back on the list screen.
- **Error boundary.** If a screen throws, the app shows a "Something went wrong" screen with the message and a Try again button that remounts the segment, instead of a blank tree. Reported to Sentry when a DSN is configured.
- **Testing tools (owner only).** Profile tab, Testing tools. Admin-only server functions that build a whole situation in one tap: a mic of yours starting in 90 minutes with a roster on it, a lottery waiting to be drawn, a week of listings across every discipline and price, an unclaimed listing plus a pending claim, three freshness states side by side, a full moderation queue, or your own account on three lists. Also a time machine that moves a night to minutes from now, a roster filler that works on real listings too, and role switches. Everything created is tracked, so one tap removes exactly it and leaves real data untouched. The whole kit sits behind an admin check plus a kill switch that is switched off before store submission. Full detail in docs/TEST_KIT.md.
- **Network (Profile tab, Network button).** Performers and producers connect by mutual consent: search by name or handle, send a request, and nothing is visible to either side until the other person accepts. Network lives under the Profile tab; a waiting request badges the Profile tab icon and the Network button shows the count. An accepted connection shows exactly two things: the other person's public socials and which upcoming mics they said they are going to (signups and "I am going" plans). There is no messaging, no feed, and no free text anywhere on a connection. Either side can remove the connection at any time (Remove on the card, confirmed before it happens); removal is immediate and silent to the other person. Report and Block work on every connection card; blocking severs the pair server side (pgTAP-tested) and prevents new requests in both directions. A "Share my nights" toggle on the screen hides all attendance from all connections at once without removing anyone. Requests are rate limited (20 per day) and both the ask and the accept push notify behind a preference that defaults on. To demo: sign in as the performer account, open Profile then Network, search for the producer account's name, connect, then accept from the producer account.
- **Invites and QR flyers (word of mouth).** "Invite friends" lives on the Profile tab, in Settings (Spread the word), and on the Network screen when a search or the connection list comes up empty; it opens the OS share sheet with a short message linking the get-the-app page, which offers the App Store and Google Play side by side (buttons stay visibly "coming soon" until the store URLs exist). Every shared flyer from the Share button carries a QR code encoding that mic's universal link: a phone with the app scans straight into the mic, everyone else lands on the mic's web page with both store options. Hosts additionally get "Signup poster" in the share sheet and a "Share this mic" entry on the Manage screen: a portrait scan-to-sign-up poster meant to be printed and hung at the venue in place of the paper list. Share telemetry stays first-party and write-only (share_events).
...
- The reset-link email flow requires the app's `openmicexplorer://` scheme; on the hosted project, add the scheme URL to the Supabase Auth redirect allowlist.
...
See README.md. With Docker: `npm run db:start`, copy the anon key into `.env`. Without Docker: `scripts/db/verify-local.sh` verifies the entire database layer (migrations, seed, 358 pgTAP tests) against system Postgres. Maestro smoke flows: `e2e/`.
