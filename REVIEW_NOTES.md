...
See README.md. With Docker: `npm run db:start`, copy the anon key into `.env`. Without Docker: `scripts/db/verify-local.sh` verifies the entire database layer (migrations, seed, 290 pgTAP tests) against system Postgres. Maestro smoke flows: `e2e/`.
