...
- **Network tab.** Performers and producers connect by mutual consent: search by name or handle, send a request, and nothing is visible to either side until the other person accepts. An accepted connection shows exactly two things: the other person's public socials and which upcoming mics they said they are going to (signups and "I am going" plans). There is no messaging, no feed, and no free text anywhere on a connection. Either side can remove the connection at any time (Remove on the card); removal is immediate and silent. Report and Block work on every connection card; blocking severs the pair server side (pgTAP-tested) and prevents new requests in both directions. A "Share my nights" toggle on the tab hides all attendance from all connections at once without removing anyone. Requests are rate limited (20 per day) and both the ask and the accept push notify behind a preference that defaults on. To demo: sign in as the performer account, search for the producer account's name, connect, then accept from the producer account.
